...
- A minimum of 32 GB RAM is required for Lase. And since we ran 15 programs 3 times in 6 modes, we suggest running them in parallel with large memory. For example, in our experiment, we use a server with 1.5 TiB memory to run 30 or 45 processes simultaneously.
...
- Setting in our experiments. All the experiments are carried out on a server equipped with an Intel Xeon Gold 6258R CPU @2.70GHz, 112 logical cores, and 1.5 TiB of RAM., and the OS is Ubuntu Linux 22.04.
...
- Pull and run the docker.
  - **docker pull lase2026/lase:latest**                      *#pull the docker images*
  - **docker images**                                                                                       *#check the docker*
  - **docker run -it lase2026/lase:latest /bin/bash**                 #run the docker
  
- A small example to run our artifact: **(KICK THE TIRES)**

  - **cd /home/Lase/DSE && ./run_RQ1_exam.sh**

    - It takes **max{(6 / CoreNum) ,1 }\* 6 + 10** mins to produce coverage results and bug detection results.
    ...
    - Please **wait for about max{(6 / CoreNum) ,1 }\* 6 mins** and the coverage results are displayed as follows.
  ...
  - **cd /home/Lase/DSE/gencov/nohup && ls**

    - We also mutate compilers to evaluate the bug detection ability of different methods.  We mutate the compiler programs and test the mutated programs by the inputs generated by Lase and the baselines. A mutant is killed when it causes the compiler to produce a different compilation result from the original version. The number of the killed mutants reflects the bug-detection capability of each method.

    - Please **wait for about 10 mins** and the bug detection results are displayed as follows. We run the benchmark `Cloli` with the inputs generated by analyzing the `C6` parser in 3 modes to explain the artifact.  The 3 modes are  ["BFS","DFS","GSDSE"]. 

      <img src="./images/bug.png" style="zoom:50%;" />

    - **tail -20 BFS_TestC6Driver_TestCLoli_bugInput_0**

      We can learn that GADSE under BFS strategy can kill 213 mutants,  among which 0 were killed by failure, 210 by exceptions, and 3 by timeout.

      ![](./images/bug-finding.png)

      The detailed results are provided in BFS_TestC6Driver_TestCLoli_bugInput_0.csv, where the status of each mutant is recorded.

    - The slight difference in the number of killed mutants may be due to the timeout setting.
  ...
  - **cd /home/Lase/grammar/vada/arvada-results && ls** 
    ...
    - You can get the grammar synthesis time in GADSE_TestRDriver_0.log (time spend building grammar).
      ...
      -  The synthesis time is 68.27s for the picture above.
...
- Run the experiments. You can choose any **one** of the following instructions.
  ...
  - **cd /home/Lase && ./run_all.sh**                                                 *#run all experiments for 1 hour*
  ...
  - Step by step:
    ...
    3. **cd /home/Lase/DSE/gencov && export JAVA_HOME=/env/jdk-11.0.21 && export PATH=\${JAVA_HOME}/bin:\$PATH  && export PATH=/env/apache-ant-1.9.16/bin:$PATH**
       - Set the environment.
       - **Note that the environment of the following command should be JAVA_HOME=/env/jdk-11.0.21.**
    4. **python3 main1.py**
       - Get the code coverage results and **generate inputs for evaluating grammar synthesis methods. You can run the grammar experiment only when you finish this step.**
    5. **python3 detect_bug.py classify && python3 detect_bug.py run**
       - Get the bug detection results.
    ...
    8. **python3 run_recall.py && python3 getRecall.py && python run_precision.py**
...
- <a name="visualization"></a>Get the tables or figures like in our paper.
  ...
  - **cd /home/Lase/DSE  && python3 count_valid_input.py**  

    - Get the average number of valid inputs derived by different methods in the procedure of symbolic execution (**Table 3 in our paper**). An example is shown below.

      <img src="./images/valid_input.png" style="zoom:38%;" />

  - **cd /home/Lase/DSE/gencov && python3 detect_bug.py display**

    - Get the bug detection results and generate bug-finding.tex.
    
  - **cd /home/Lase/grammar/lase && python3 processData.py && cd /home/Lase/grammar/vada && python3 processData.py**

    - Generate the files, results0, results1, results2 and results3, for evaluating the token-level synthesis method and Exchange operation proposed in Lase. results0, results1, results2, results3 save the results under 4 modes, Lase (Glade equipped with the token-level synthesis method and Exchange operation), Glade-t (Glade equipped with the token-level synthesis method), Glade-c (vanilla Glade) and Lase-c (Glade equipped with Exchange operation), respectively.
...
- We provide only the 1-hour experimental commands here. To perform 12-hour experiments, please replace **python3 main.py** with  **python3 main_12h.py**  in the above commands.

- Reproduce the tables and figures presented in our paper.

  - To reproduce the tables and figures presented in our paper, We already saved the running results in our paper to the docker. Specifically, you can follow the instructions below to reproduce the results in our paper.
    - **cd /home/Lase_display_1h**
  - run the visualization scripts [above](#visualization) and replace `Lase` with `Lase_display_1h` in the instructions. If you want to get the results of 12-hour experiments, replace `Lase` with `Lase_display_12h`.
    - For example, "cd /home/**Lase**/DSE/gencov && python3 computeAver.py" should be replaced with "cd /home/**Lase_display_1h**/DSE/gencov && python3 computeAver.py".
...
- **For RQ1** (the effectiveness of online grammar synthesis and our new search heuristic), we generate result1.tex, result2.tex and newCmd.tex.
  ...
  - result1.tex presents the coverage results of a one hour running in different modes on 15 parsers, which is presented as a table like the following one (section 4 of our paper). 
    ...
    <img src="./images/result1.png" style="zoom:50%;" />

    - Inside the table, prog represents the benchmarks. B and S are branch coverage and statement coverage, respectively, which are the metrics for effectiveness evaluation. DFS and BFS are the baseline method GADSE under the DFS strategy and BFS strategy, respectively. Lase implements our proposed method, which incorporates both the new search strategy and grammar synthesis. And we show the results of GADSE equipped with our new search strategy (NS) and grammar synthesis modular under DFS and BFS strategies (GS#D and GS#B) to inspect the influence of each part. We highlight the best results with dark gray and the second-best results with light gray to inspect the primary influence factors.
  ...
  - result2.tex can generate a figure like the following one (section 4 of our paper).
    ...
    <img src="./images/result2.png" style="zoom:50%;" />
...
- **Bug detection.**  
  ...
  - bug-finding.tex can generate a figure as follows.

    ![](./images/bug-result.png)
...
- **For RQ2** (the effectiveness and efficiency of our token-level grammar synthesis method compared with the state-of-the-art methods), we generate experimental_results.tex and execution_time.tex.
  ...
  - experimental_results.tex can generate a table as shown below.
    ...
    <img src="./images/grammar.png" style="zoom:50%;" />
  ...
  - execution_time.tex can generate a table like the following one.
    ...
    <img src="./images/time.png" style="zoom:50%;" />
    ...
    - We use the synthesis time to inspect the efficiency of Lase and other grammar synthesis methods.
